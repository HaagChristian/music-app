...
from src.api.myapi.registration_model import SignUpRequestModel, SignUpUserResponse, AuthUser
...
def register_user(user_model: SignUpRequestModel, db: Session) -> SignUpUserResponse:
    ...
    return SignUpUserResponse(first_name=user_model.first_name, last_name=user_model.last_name, email=user_model.email,
                              username=user_model.username)
    # Do not return the password hash or any sensitive information


def signin_user(email, password, db: Session) -> AuthUser:
